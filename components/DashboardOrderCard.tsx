...
export default function DashboardOrderCard({ order }: { order: WorkOrderRow }) {
  const tone = severityTone[order.severity] ?? "neutral";
  const critical = order.severity === "critical" && order.status !== "closed";

  return (
    <div
      className={cn(
        "overflow-hidden rounded-lg border p-3.5 text-[12.5px] transition-colors",
        critical
          ? "border-severe-line bg-severe-bg"
          : "border-line-1 bg-surface-1 hover:border-line-2",
      )}
    >
